---
title: "Fitting Continuous Distributions with distfit"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Fitting Continuous Distributions with distfit}
...
The `distfit()` function provides a unified interface for fitting univariate
...
fit_egpd <- distfit(x, type = 1, family = "egpd")
summary(fit_egpd)
```
...
aic_table <- data.frame(
  type = c(1, 4, 5),
  AIC = c(
    AIC(distfit(x, type = 1)),
    AIC(distfit(x, type = 4)),
    AIC(distfit(x, type = 5))
  )
)
aic_table
```
...
```{r egpd-fix}
fit_fix <- distfit(x, type = 1, fix.arg = list(xi = 0.1))
summary(fit_fix)
```
...
aic_m <- sapply(c(4, 6, 8, 12), function(m) {
  AIC(distfit(x, type = 1, method = "bernstein", bernstein.m = m))
})
...
```{r cpegpd-fit}
fit_cp <- distfit(y, type = 1, family = "cpegpd", cpegpd.h = 0.1)
summary(fit_cp)
```
...
aic_h <- sapply(h_grid, function(hv) {
  AIC(distfit(y, type = 1, family = "cpegpd", cpegpd.h = hv))
})
...
```{r cpegpd-fix}
fit_cp_fix <- distfit(y, type = 1, family = "cpegpd",
