---
title: "Predictive Coverage Assessment"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Predictive Coverage Assessment}
...
A well-calibrated model should produce prediction intervals whose empirical
coverage matches their nominal level.  The `predictive_coverage()` function
computes simulation-based prediction intervals for fitted EGPD models and
reports their empirical coverage.  This vignette demonstrates its use for
all three fitting approaches --- `egpd()`, `gamlss()`, and `bamlss()` ---
and shows how to construct calibration plots that compare nominal and
empirical coverage across a range of levels.
...
```{r fit-bamlss, results = "hide", warning = FALSE}
...
estimates) and **parametric** prediction (parameters perturbed on the link
scale to account for estimation uncertainty).

```{r coverage-95, cache = TRUE}
set.seed(1)
nsim <- 3000
...
All approaches achieve empirical coverage close to the nominal 95% level.
The parametric method tends to produce slightly wider intervals because it
...
```{r calibration-plugin-plot, fig.width = 6, fig.height = 6}
...
```{r calibration-parametric-plot, fig.width = 6, fig.height = 6}
plot(levels, levels, type = "l", lwd = 1, col = "grey50", lty = 2,
     xlab = "Nominal level", ylab = "Empirical coverage",
...
EGPD data where the scale parameter varies smoothly with a covariate `x`.
...
```{r smooth-fit, warning = FALSE, results = "hide"}
...
```{r smooth-calibration, cache = TRUE}
set.seed(3)
...
```{r smooth-calibration-plot, fig.width = 6, fig.height = 6}
...
stringent test is **out-of-sample** (forecast) coverage, where we hold out
observations that the model has never seen.  This is the relevant metric
for real forecasting applications.
...
```{r forecast-fit, warning = FALSE, results = "hide"}
...
```{r forecast-coverage-95, cache = TRUE}
set.seed(5)
nsim_f <- 3000
...
```{r forecast-calibration, cache = TRUE}
set.seed(6)
...
```{r forecast-cal-plugin-plot, fig.width = 6, fig.height = 6}
...
```{r forecast-cal-parametric-plot, fig.width = 6, fig.height = 6}
plot(levels_f, levels_f, type = "l", lwd = 1, col = "grey50", lty = 2,
     xlab = "Nominal level", ylab = "Empirical coverage",
...
```{r forecast-interval-plot, fig.width = 8, fig.height = 6}
...
parameter.  Held-out observations that fall outside the interval (red) are
roughly 5% of the total, matching the nominal level.

### In-sample vs out-of-sample comparison

```{r in-vs-oos, cache = TRUE}
set.seed(7)
...
```{r shortest-vs-equal, cache = TRUE}
set.seed(4)
...
- The **parametric** method accounts for estimation uncertainty and tends
  to yield slightly wider intervals than the **plug-in** method.
...
- Calibration plots are a useful diagnostic: deviations from the diagonal
  signal model misspecification or inadequate uncertainty quantification.
