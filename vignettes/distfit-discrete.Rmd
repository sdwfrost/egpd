---
title: "Fitting Discrete Distributions with distfit"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Fitting Discrete Distributions with distfit}
...
The `distfit()` function supports two discrete distribution families for
...
```{r degpd-fit}
fit_degpd <- distfit(x, type = 1, family = "degpd")
summary(fit_degpd)
```
...
a normal Q-Q plot of randomized quantile residuals (panel 3), and a P-P
plot using the randomized probability integral transform (panel 4). The
randomized PIT (Dunn & Smyth, 1996) spreads tied discrete values into
continuous uniform variates, producing clean diagonal plots instead of
the banded patterns that arise from integer-valued quantiles.
...
aic_table <- data.frame(
  type = c(1, 4, 5),
  AIC = c(
    AIC(distfit(x, type = 1, family = "degpd")),
    AIC(distfit(x, type = 4, family = "degpd")),
    AIC(distfit(x, type = 5, family = "degpd"))
  )
)
aic_table
```
...
```{r degpd-fix}
fit_fix <- distfit(x, type = 1, family = "degpd",
                    fix.arg = list(xi = 0.1))
summary(fit_fix)
```
...
```{r zidegpd-fit}
fit_zi <- distfit(y, type = 1, family = "zidegpd")
summary(fit_zi)
```
...
```{r compare-zi}
fit_no_zi <- distfit(y, type = 1, family = "degpd")
...
fit_zi_on_x <- distfit(x, type = 1, family = "zidegpd")
...
x5 <- rdiscegpd(1000, sigma = 2, xi = 0.1, delta = 1.2,
                 kappa = 1.8, type = 5)
fit5 <- distfit(x5, type = 5, family = "degpd")
summary(fit5)
```
...
```{r cpdegpd-fit}
fit_cpdegpd <- distfit(z, type = 1, family = "cpdegpd")
summary(fit_cpdegpd)
```
...
points, a step-function CDF, and randomized PIT-based Q-Q and P-P plots.
...
```{r cpdegpd-compare}
fit_degpd_on_z <- distfit(z, type = 1, family = "degpd")
...
- Return S3 objects of class `"distfit"` with the same interface
...
- Provide four-panel diagnostic plots tailored for discrete data
  (bar plots for PMF, step functions for CDF, randomized PIT-based
  Q-Q and P-P plots)
