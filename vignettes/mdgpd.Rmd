...
5. **Neural Bayes estimation** -- fitting via `distfit()`
...
```{r fit-mdgpd-npe, eval = has_julia_support}
set.seed(42)

Y <- rmdgpd(1000, sigma = 2, xi = 0.2, lambda = 1, rho = 0.5)
fit <- distfit(Y, family = "mdgpd", method = "neuralbayes",
               estimator = "npe", nsamples = 2000)
summary(fit)
```

```{r fit-mdgpd-diagnostics, eval = has_julia_support, fig.width = 8, fig.height = 8}
plot(fit)
```

### Fitting ZIMDGPD

```{r fit-zimdgpd-npe, eval = has_julia_support}
set.seed(42)

Y_zi <- rzimdgpd(1000, sigma = 2, xi = 0.2, lambda = 1, rho = 0.5,
                  pi0 = 0.3)
fit_zi <- distfit(Y_zi, family = "zimdgpd", method = "neuralbayes",
                   estimator = "npe")
summary(fit_zi)
```
...
```{r s3-mdgpd, eval = has_julia_support}
...
```{r fit-3d, eval = has_julia_support}
...
fit_3d <- distfit(Y3_fit, family = "mdgpd", method = "neuralbayes",
