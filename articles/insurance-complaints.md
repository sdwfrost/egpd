...
![](insurance-complaints_files/figure-html/eda-1.png)
...
The [`egpd()`](https://sdwfrost.github.io/egpd/reference/egpd.md)
function fits DEGPD models within a GAM framework. For intercept-only
models (no covariates), each parameter gets a `~ 1` formula. The DEGPD
family uses a log link for the scale (`sigma`) and shape (`xi`)
...
![](insurance-complaints_files/figure-html/gof-1.png)
...
function computes these by drawing a uniform variate between the lower
and upper CDF bounds at each observation and transforming to the normal
scale.
...
![](insurance-complaints_files/figure-html/qq-1.png)
