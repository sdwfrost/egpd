...
simulates `n_rep` datasets, fits each with `egpd()`, uses `confint()` to
obtain Wald 95% CIs on the response scale, and checks whether the true
values are covered.
...
`egpd()` achieve empirical coverage close to the nominal 95% level. Key
findings:
...
The `confint()` method for egpd objects supports both Wald and profile
likelihood confidence intervals. Profile likelihood CIs invert the
likelihood ratio test and do not rely on the normal approximation, so
they can provide better coverage when the log-likelihood surface is
asymmetric or the sample size is moderate.
...
confidence intervals from `egpd()` achieve empirical coverage close to
the nominal 95% level. Key findings:
