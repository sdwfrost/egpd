...
methods that use maximum likelihood, the bivariate BEGPD uses
pre-trained neural networks to perform amortized inference – either
posterior sampling (NPE) or point estimation (NBE).
...
The BEGPD jointly models two variables $`(Y_1, Y_2)`$ by decomposing
them into a radial (intensity) component and angular (dependence)
components.
...
where $`\kappa > 0`$ is the EGPD shape, $`\sigma > 0`$ is the GPD scale,
and $`\xi > 0`$ is the GPD shape parameter. The CDF of $`R`$ is the EGPD
CDF:
...
The [`rbegpd()`](https://sdwfrost.github.io/egpd/reference/rbegpd.md)
function generates bivariate samples from the BEGPD. It is pure R and
...
![](multivariate-egpd_files/figure-html/scatter-basic-1.png)
...
![](multivariate-egpd_files/figure-html/scenarios-scatter-1.png)
...
- **Scenario 3** ($`\xi=0.2`$, $`\theta_L=4`$, $`\theta_U=0.5`$):
  Heavier GPD tail ($`\xi=0.2`$) and stronger dependence in both tails.
...
![](multivariate-egpd_files/figure-html/chi-measures-1.png)
...
#> kappa 4.3055  2.4573 1.0860 10.1771
#> sigma 0.5708  0.2875 0.2719  1.3542
#> xi    0.1865  0.1071 0.0205  0.4066
#> thL   6.9177  5.7649 0.4851 21.5516
#> thU   4.9914  2.8001 2.2984 13.1253
#> thw   0.2609  0.1414 0.0341  0.5621
#> 
...
produces a 4-panel diagnostic plot adapted for bivariate data:
...
![](multivariate-egpd_files/figure-html/npe-plot-1.png)
...
3.  **Radial Q-Q plot** – quantiles of $`Y_1+Y_2`$ (observed vs
    simulated)
...
#> 4.3054671 0.5707785 0.1864659 6.9177241 4.9913715 0.2608946

# Variance-covariance (from posterior sample covariance for NPE)
vcov(fit_npe)
#>            kappa        sigma            xi         thL          thU
#> kappa  6.0383610 -0.569099733  0.1334242775  1.12679903  0.469468894
#> sigma -0.5690997  0.082640062 -0.0210026769  0.03389059 -0.144149272
#> xi     0.1334243 -0.021002677  0.0114692329 -0.01135606  0.007713971
#> thL    1.1267990  0.033890591 -0.0113560599 33.23370961 -9.582361555
#> thU    0.4694689 -0.144149272  0.0077139708 -9.58236155  7.840778003
#> thw    0.0404016 -0.005249683  0.0001356025  0.02719962  0.099818408
#>                 thw
#> kappa  0.0404015978
#> sigma -0.0052496834
#> xi     0.0001356025
...
confint(fit_npe)
#>            2.5 %     97.5 %
#> kappa 1.08600938 10.1771322
#> sigma 0.27193225  1.3542142
#> xi    0.02053126  0.4065600
#> thL   0.48505560 21.5515754
#> thU   2.29839724 13.1252676
#> thw   0.03413956  0.5621031
confint(fit_npe, level = 0.90)
#>              5 %       95 %
#> kappa 1.35943784  9.0037728
#> sigma 0.29724626  1.1779225
#> xi    0.03094663  0.3741902
#> thL   0.89918911 19.0362849
#> thU   2.64144828 10.9145860
#> thw   0.05179912  0.5176432

# Subset parameters
confint(fit_npe, parm = c("kappa", "xi"))
#>            2.5 %   97.5 %
#> kappa 1.08600938 10.17713
#> xi    0.02053126  0.40656
```
...
#> kappa  2.0   4.3055 1.0860 10.1771    TRUE
#> sigma  1.0   0.5708 0.2719  1.3542    TRUE
#> xi     0.1   0.1865 0.0205  0.4066    TRUE
#> thL    5.0   6.9177 0.4851 21.5516    TRUE
#> thU    5.0   4.9914 2.2984 13.1253    TRUE
#> thw    0.2   0.2609 0.0341  0.5621    TRUE
```
...
print(round(rmse, 4))
#>  kappa  sigma     xi    thL    thU    thw 
#> 2.2678 0.4166 0.0845 1.9226 0.6996 0.0566
```
...
2.  Simulates bivariate data from each parameter set using
    [`rbegpd()`](https://sdwfrost.github.io/egpd/reference/rbegpd.md)
...
- **$`\psi`$ (encoder)**: Processes each 2-dimensional observation
  through shared Dense layers with residual connections and LayerNorm
- **$`\phi`$ (aggregator)**: Aggregates the encoded representations
  using mean pooling, concatenates an expert summary statistic
