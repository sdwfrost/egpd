...
function provides a unified interface for fitting univariate EGPD models
by maximum likelihood. This vignette covers the three continuous
families:
...
![](fitegpd-continuous_files/figure-html/egpd-plot-1.png)
...
The reported parameters are only $`\sigma`$, $`\xi`$, and $`\kappa`$;
the Bernstein weights are stored internally. The effective number of
...
![](fitegpd-continuous_files/figure-html/bern-plot-1.png)
...
- $`X_i \sim \mathrm{EGPD}(\sigma, \xi, \kappa)`$ are i.i.d. event
  sizes.
...
The [`rcpegpd()`](https://sdwfrost.github.io/egpd/reference/rcpegpd.md)
function uses direct simulation (no Panjer needed): draw
...
![](fitegpd-continuous_files/figure-html/cpegpd-hist-1.png)
...
![](fitegpd-continuous_files/figure-html/cpegpd-plot-1.png)
...
![](fitegpd-continuous_files/figure-html/cpegpd-cdf-check-1.png)
