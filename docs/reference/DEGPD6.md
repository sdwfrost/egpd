...
The `DEGPD6()` function defines a `gamlss.family` distribution for
fitting a discrete Extended Generalized Pareto Distribution with model 6
(truncated beta G-transformation).

The functions `dDEGPD6`, `pDEGPD6`, `qDEGPD6`, and `rDEGPD6` define the
PMF, distribution function, quantile function, and random generation.

## Usage

``` r
DEGPD6(mu.link = "log", sigma.link = "log", nu.link = "log")

dDEGPD6(x, mu = 1, sigma = 0.5, nu = 1, log = FALSE)
pDEGPD6(q, mu = 1, sigma = 0.5, nu = 1, lower.tail = TRUE, log.p = FALSE)
qDEGPD6(p, mu = 1, sigma = 0.5, nu = 1, lower.tail = TRUE, log.p = FALSE)
rDEGPD6(n, mu = 1, sigma = 0.5, nu = 1)
```
...
`DEGPD6()` returns an object of class `"gamlss.family"`.

`dDEGPD6` gives the PMF, `pDEGPD6` gives the distribution function,
`qDEGPD6` gives the quantile function, and `rDEGPD6` generates random
deviates.
...
if (FALSE) { # \dontrun{
  library(gamlss)
  set.seed(1)
  y <- rDEGPD6(500, mu = 3, sigma = 0.15, nu = 2)
