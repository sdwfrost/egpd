...
// Reparameterization: ldkappa = log(kappa2 - kappa1), compute lkappa2 = log(kappa1 + exp(ldkappa)) via log-sum-exp
{
...
// r1 = kappa1/kappa2, r2 = 1 - r1 = (kappa2 - kappa1)/kappa2
{
  ...
  // Cross terms with param 0 (lpsi): H(0,3)=col7, H(0,4)=col8
  double H03 = out(j, 7), H04 = out(j, 8);
  out(j, 7) = H03 + H04 * r1;
  out(j, 8) = H04 * r2;

  // Cross terms with param 1 (xi): H(1,3)=col11, H(1,4)=col12
...
if (off2.n_elem > 0) lkappa1vec += off2;
arma::vec off3 = Rcpp::as<arma::vec>(offsets[3]);
if (off3.n_elem > 0) lkappa2vec += off3;
arma::vec off4 = Rcpp::as<arma::vec>(offsets[4]);
if (off4.n_elem > 0) logitpvec += off4;
}
