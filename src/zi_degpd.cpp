...
double zidegpd2d0(const Rcpp::List& pars, const arma::mat& X1, const arma::mat& X2, const arma::mat& X3, const arma::mat& X4 , const arma::mat& X5, const arma::mat& X6, arma::vec yvec, const arma::uvec& dupid, int dcate, const Rcpp::List& offsets)
{
  ...
  {
  arma::vec off_i = Rcpp::as<arma::vec>(offsets[2]);
  if (off_i.n_elem > 0) lkappa1vec += off_i;
  }
  {
  arma::vec off_i = Rcpp::as<arma::vec>(offsets[3]);
  if (off_i.n_elem > 0) ldkappavec += off_i;
  }
  {
  arma::vec off_i = Rcpp::as<arma::vec>(offsets[4]);
  if (off_i.n_elem > 0) logitpvec += off_i;
  }
  {
  arma::vec off_i = Rcpp::as<arma::vec>(offsets[5]);
  if (off_i.n_elem > 0) logitpivec += off_i;
  }
...
// 6-param column indices
{
  ...
  // Cross terms with param 0 (lsigma): H(0,3)=col8, H(0,4)=col9
  double H03 = out(j, 8), H04 = out(j, 9);
  out(j, 8) = H03 + H04 * r1;
  out(j, 9) = H04 * r2;

  // Cross terms with param 1 (lxi): H(1,3)=col13, H(1,4)=col14
  double H13 = out(j, 13), H14 = out(j, 14);
  out(j, 13) = H13 + H14 * r1;
  out(j, 14) = H14 * r2;

  // Diagonal block: H(3,3)=col17, H(3,4)=col18, H(4,4)=col21
  double H33 = out(j, 17), H34 = out(j, 18), H44 = out(j, 21);
  out(j, 17) = H33 + 2.0 * r1 * H34 + r1 * r1 * H44 + g4 * r1 * r2;
  out(j, 18) = r2 * H34 + r1 * r2 * H44 - g4 * r1 * r2;
  out(j, 21) = r2 * r2 * H44 + g4 * r1 * r2;

  // Cross terms with param 4 (logitp): H(4,3)=col19, H(4,4)=col22
  double H43 = out(j, 19), H44p = out(j, 22);
  out(j, 19) = H43 + H44p * r1;
  out(j, 22) = H44p * r2;

  // Cross terms with param 5 (logitpi): H(5,3)=col20, H(5,4)=col23
  double H53 = out(j, 20), H54 = out(j, 23);
  out(j, 20) = H53 + H54 * r1;
  out(j, 23) = H54 * r2;
}
...
double zidegpd3d0(const Rcpp::List& pars, const arma::mat& X1, const arma::mat& X2, const arma::mat& X3, const arma::mat& X4, arma::vec yvec, const arma::uvec& dupid, int dcate, const Rcpp::List& offsets)
{
  ...
  {
  arma::vec off_i = Rcpp::as<arma::vec>(offsets[2]);
  if (off_i.n_elem > 0) ldeltavec += off_i;
  }
  {
  arma::vec off_i = Rcpp::as<arma::vec>(offsets[3]);
  if (off_i.n_elem > 0) logitpivec += off_i;
  ...
  //  # second derivatives
    out(j, 5) = -(((0.5 * 
                      ((((ee38 * R_pow(ee10,ee253) * ee12 - ee244)/ee3 - ee244) * 
                          ee41 * ee1 + R_pow(ee82,2) * ee141 * ee32) * R_pow(ee5,2)) - 0.5 * 
