## Tests for distfit() MLE fitting

test_that("distfit type 1 continuous EGPD recovers parameters", {
  set.seed(42)
  true_sigma <- 2; true_xi <- 0.1; true_kappa <- 1.5
  x <- regpd(500, sigma = true_sigma, xi = true_xi, kappa = true_kappa, type = 1)
  fit <- distfit(x, type = 1, family = "egpd")
...
test_that("distfit type 4 continuous EGPD converges", {
  set.seed(123)
  x <- regpd(300, sigma = 1.5, xi = 0.2, delta = 1.2, type = 4)
  fit <- distfit(x, type = 4, family = "egpd")
...
test_that("distfit type 5 continuous EGPD converges", {
  set.seed(456)
  x <- regpd(300, sigma = 2, xi = 0.1, delta = 1, kappa = 2, type = 5)
  fit <- distfit(x, type = 5, family = "egpd")
...
test_that("distfit discrete EGPD type 1 converges", {
  set.seed(789)
  x <- rdiscegpd(500, sigma = 3, xi = 0.1, kappa = 1.5, type = 1)
  fit <- distfit(x, type = 1, family = "degpd")
...
test_that("distfit zero-inflated discrete EGPD type 1 converges", {
  set.seed(101)
  x <- rzidiscegpd(500, pi = 0.3, sigma = 2, xi = 0.1, kappa = 1.5, type = 1)
  fit <- distfit(x, type = 1, family = "zidegpd")
...
test_that("distfit zero-inflated continuous EGPD type 1 converges", {
  set.seed(202)
  x <- rziegpd(500, pi = 0.2, sigma = 2, xi = 0.1, kappa = 1.5, type = 1)
  fit <- distfit(x, type = 1, family = "ziegpd")
...
test_that("distfit fix.arg works correctly", {
  set.seed(42)
  x <- regpd(300, sigma = 2, xi = 0.05, kappa = 1.5, type = 1)
  fit <- distfit(x, type = 1, fix.arg = list(xi = 0.05))
...
test_that("distfit S3 methods work", {
  set.seed(42)
...
test_that("distfit plot does not error", {
  set.seed(42)
  x <- regpd(200, sigma = 2, xi = 0.1, kappa = 1.5, type = 1)
  fit <- distfit(x, type = 1)
...
test_that("distfit input validation works", {
  expect_error(distfit(1, type = 1), "'x' must be a numeric")
  expect_error(distfit(c(1, 2, 3), type = 7), "'type' must be an integer")
  expect_error(distfit(c(1, 2, 3), type = 1, method = "bernstein", family = "degpd"),
               "Bernstein method is only available")
  expect_error(distfit(c(1, 2, 3), type = 1, fix.arg = list(foo = 1)),
               "Unknown fixed parameters")
})

test_that("distfit with custom start values works", {
  set.seed(42)
  x <- regpd(300, sigma = 2, xi = 0.1, kappa = 1.5, type = 1)
  fit <- distfit(x, type = 1, start = list(sigma = 1.5, kappa = 1.2))

  expect_equal(fit$convergence, 0)
})

test_that("distfit type 6 continuous EGPD converges", {
  set.seed(999)
  x <- regpd(500, sigma = 2, xi = 0.1, kappa = 1.5, delta = 2, prob = 0.4, type = 6)
  fit <- distfit(x, type = 6, family = "egpd")
...
test_that("deprecated fitegpd() alias warns and delegates to distfit()", {
  set.seed(1)
  x <- regpd(400, sigma = 2, xi = 0.1, kappa = 1.5, type = 1)
  fit_new <- distfit(x, type = 1, family = "egpd")
  expect_warning(fit_old <- fitegpd(x, type = 1, family = "egpd"), "deprecated")
  ## same class and same estimates as the new name
  expect_s3_class(fit_old, "distfit")
  expect_equal(fit_old$estimate, fit_new$estimate)
})
